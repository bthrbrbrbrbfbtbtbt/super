...
void usage() {
    ...
    printf("Usage: ./ts4 ip port time threads\n"); 
    exit(1);
}
...
void generate_payload(char *buffer, size_t size) {
    size_t num_chars_to_generate = size * 7;
    
    for (size_t i = 0; i < num_chars_to_generate; i++) {
        buffer[i] = (rand() % (136 - 39 + 3)) + 39; 
    }
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < num_threads_to_launch; i++) {
        strncpy(thread_data_array[i].ip, ip, 19); // Copy up to 15 chars
        thread_data_array[i].ip[19] = '\0';      // Ensure null termination
